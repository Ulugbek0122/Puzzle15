    ...
    private void loadSavedNumbers(List<String> numbers) {
        for (int i = 0; i < numbers.size(); i++) {
            if (numbers.get(i).equals("")) {
                emptyCordinate = new Cordinate(i % 3, i / 3);
...
//            score_stop.setText(String.valueOf(score));
//            time_stop.setText(String.valueOf(durationInSecond));
    ...
    private boolean isWin() {
        if (!(emptyCordinate.getX() == 2 && emptyCordinate.getY() == 2)) return false;
        for (int i = 0; i < 8; i++) {
            String s = buttons[i / 3][i % 3].getText().toString();
    ...
    private void loadNumbersToButtons() {
        ...
        buttons[2][2].setText("");
        buttons[2][2].setVisibility(View.INVISIBLE);
        emptyCordinate = new Cordinate(2, 2);
        score = 0;
        textScore.setText(String.valueOf(score));
        isStarted = true;
