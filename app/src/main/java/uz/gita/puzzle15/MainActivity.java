...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sound_isOn = sharedPreferences.getBoolean("sound_isOn", this.sound_isOn);
    ...
    private void loadSavedNumbers(List<String> numbers) {
        for (int i = 0; i < numbers.size(); i++) {
            if (numbers.get(i).equals("")) {
                emptyCordinate = new Cordinate(i % 4, i / 4);
...
//            time_stop.setText(String.valueOf(durationInSecond));
    ...
    private boolean isWin() {
        if (!(emptyCordinate.getX() == 3 && emptyCordinate.getY() == 3)) return false;
        for (int i = 0; i < 15; i++) {
    ...
    private void loadNumbersToButtons() {
        ...
        emptyCordinate = new Cordinate(3, 3);
        score = 0;
        textScore.setText(String.valueOf(score));
        isStarted = true;
